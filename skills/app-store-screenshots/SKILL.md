...
- **Apple Watch** (portrait, Ultra 3 / Series 11 / Series 9 / Series 6 / Series 3) — Apple App Store, required only when the app ships a watchOS target
...
// Apple Watch — designed at the Ultra 3 max (422×514). All smaller
// Series sizes are exported by re-rendering at their native res.
const WATCH_W = 422;  const WATCH_H = 514;
...
#### Apple Watch (Apple required when the app ships a watchOS extension, portrait)

```typescript
// Apple lets you drag up to 10 watch screenshots per size. The
// generator emits one PNG per slide at each native resolution.
const WATCH_SIZES = [
  { label: 'Ultra 3 (49mm)',  w: 422, h: 514 },
  { label: 'Ultra (49mm legacy)', w: 410, h: 502 },
  { label: 'Series 11 (46mm)', w: 416, h: 496 },
  { label: 'Series 9 (45mm)',  w: 396, h: 484 },
  { label: 'Series 6 (44mm)',  w: 368, h: 448 },
  { label: 'Series 3 (38mm)',  w: 312, h: 390 },
] as const;
```

Apple Watch screenshots are **tiny canvases** (under 500 px). Captions
that work on iPhone (font-size ≈ 0.085 × canvas width) overwhelm the
slide. Treat Watch slides as "one line of headline + watch frame
centered" — no decoration, no long copy. Headline ≤ 4 words.
...
type Device =
  | "iphone"
  | "ipad"
  | "watch"          // ← Apple Watch (Ultra 3 → Series 3)
  | "android"
  | "android-7"
  | "android-10"
  | "feature-graphic";
...
const WATCH_RATIO = 0.82;        // Apple Watch frame — averaged across
                                  // Ultra 3 (0.821), S11 (0.839), S9
                                  // (0.818), S6 (0.821), S3 (0.800).
                                  // One CSS frame fits every model.
```
...
// Apple Watch — tiny canvas (~422×514). Clamp aggressively at 0.62
// so the watch frame doesn't crowd the caption. Anything bigger and
// the headline has nowhere to land.
function watchW(cW: number, cH: number, clamp = 0.62) {
  return Math.min(clamp, 0.65 * (cH / cW) * WATCH_RATIO);
}
...
#### Apple Watch (CSS-only)

One CSS frame covers Ultra 3, Series 11, Series 9, Series 6, and
Series 3 — they all share a rounded-rectangle silhouette at ≈ 0.82
aspect. Render the digital crown nub on the right and a small
side-button stub below it; everything else is corner radius.

```tsx
function AppleWatch({ src, alt, style }: { src: string; alt: string; style?: React.CSSProperties }) {
  return (
    <div style={{ position: "relative", aspectRatio: `${WATCH_RATIO}`, ...style }}>
      <div style={{
        width: "100%", height: "100%",
        // Watch corners are very round — ~22% of width.
        borderRadius: "22% / 18%",
        background: "linear-gradient(160deg, #1F1F22 0%, #0B0B0E 100%)",
        boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.10), 0 12px 36px rgba(0,0,0,0.55)",
        position: "relative", overflow: "hidden",
      }}>
        {/* Digital Crown — right side, upper third */}
        <div style={{
          position: "absolute",
          right: "-2.2%", top: "26%",
          width: "5%", height: "10%",
          borderRadius: "30% / 50%",
          background: "linear-gradient(90deg, #3a3a3e 0%, #1a1a1c 100%)",
          border: "1px solid rgba(255,255,255,0.08)",
          zIndex: 20,
        }} />
        {/* Side button — right side, lower third */}
        <div style={{
          position: "absolute",
          right: "-1.4%", top: "55%",
          width: "3.6%", height: "12%",
          borderRadius: "30% / 60%",
          background: "linear-gradient(90deg, #2a2a2c 0%, #141416 100%)",
          zIndex: 20,
        }} />
        {/* Screen — inset, very rounded */}
        <div style={{
          position: "absolute",
          left: "5%", top: "5%",
          width: "90%", height: "90%",
...
const WATCH_SLIDES       = [makeSlide1(AppleWatch, watchW, "screenshots/apple/watch", WATCH_RATIO), ...];
```

For Apple Watch slides, lay out caption ABOVE the watch with very tight
spacing — the canvas is too small (~422×514) for the iPhone "caption +
bottom phone" pattern. Use 1 short line of headline (≤ 4 words) +
watch centered horizontally. Drop label/eyebrow text — there's no
room.
...
const { cW, cH, currentSizes, slides } = (() => {
  ...
  if (device === "watch")           return { cW: WATCH_W,cH: WATCH_H,currentSizes: WATCH_SIZES,     slides: WATCH_SLIDES };
...
<div style={{ position: "sticky", top: 0, zIndex: 50, background: "white", borderBottom: "1px solid #e5e7eb", display: "flex", alignItems: "center" }}>
  ...
  <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", overflowX: "auto", minWidth: 0 }}>
    ...
    <div style={{ display: "flex", gap: 4, background: "#f3f4f6", borderRadius: 8, padding: 4, flexShrink: 0 }}>
      {(["iphone", "android", "ipad", "watch", "feature-graphic"] as Device[]).map(d => (
        <button key={d} onClick={() => { setDevice(d); setSizeIdx(0); }}
          ...
          {d === "iphone" ? "iPhone" : d === "android" ? "Android" : d === "ipad" ? "iPad" : d === "watch" ? "Watch" : "Feature Graphic"}
        </button>
      ))}
...
| Apple Watch caption overflows | Watch canvas is < 500px wide — drop eyebrow label, shrink headline to ≤ 4 words, no decorative elements |
| Apple Watch frame too round / too square | Use `WATCH_RATIO = 0.82` aspect + `borderRadius: "22% / 18%"` — one CSS frame covers Ultra 3 → Series 3 |
