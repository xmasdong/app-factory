...
> ```
> 线性 Step(下方段一~段三)仅作**单 agent 降级档**(无 Workflow runtime 时手跑)。见末尾【## 主执行路径:AI 调用 Workflow 工具编排(推荐用户开 ultracode;降级=单 agent 顺序)】。
...
`降级: 无 Workflow 工具可用(本会话非 ultracode/未提供该工具),已退线性段一~段三单 agent 顺序跑,两份 state JSON 手写(key 仍对齐闸门)`
