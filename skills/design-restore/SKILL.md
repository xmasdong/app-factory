...
> ⚠️ **还原真 app 时不走线性 Step:** 推荐用户开 ultracode,由 AI 用内置 Workflow 工具(归 Claude)按本 skill 描述的编排意图现场组合并执行(扇出哪些屏/视口、各 agent 干啥见末节)。`scripts/design-first/design-restore.workflow.js` 是【编排蓝图参考】,展示推荐扇出结构供 AI/人参考,不是传给工具去跑的 script。线性 Step(段一~段三)= 未做扇出编排时的单 agent 降级档。见末尾【## 主执行路径:AI 用内置 Workflow 工具按编排意图现场组合(推荐用户开 ultracode;降级=单 agent 顺序)】。
...
- **扇出编排时**额外要求:`CLAUDE_PROJECT_DIR` 已 export 指向目标项目根(否则 ROOT 落到 cwd,state JSON 写错位置闸门读不到)
...
- `CLAUDE_PROJECT_DIR` 未设 → 编排里的 agent 写盘会退用 `process.cwd()`,提示用户确认 cwd 是项目根
...
- **闸门 state(由汇总/critic agent 唯一写入,key 严格对齐 `app-gate.sh`):**
...
> **真 app:由 AI 按本 skill 编排意图用内置 Workflow 工具扇出执行(见末节)。** 以下线性 Step = 降级档(单 agent 手跑)。

```
真 app(默认):
  AI 按末节编排意图,用内置 Workflow 工具现场组合扇出执行
  (蓝图参考: scripts/design-first/design-restore.workflow.js)

降级档(未做扇出编排时单 agent 顺序跑):
段一:抽取(归一化)
...
> **蓝图对应:** 蓝图里的 `Extract` 阶段是单 agent —— 扇出前唯一输入,产 `screens[] + dpr`。
...
→ **产出:** `extraction_meta`(进 design-manifest.json),对应蓝图里 `extract` agent 返回对象的一部分。
...
```
→ **产出:** `docs/design/tokens.json`(对应蓝图 `extract` agent 写)。
...
→ **产出:** `docs/design/design-manifest.json`(对应蓝图 `extract` agent 写,返回 `screens[] + dpr`)。
...
> **蓝图对应:** 此段在蓝图 `Per-Screen` worker 的"局部重生"子步内被各屏消费;token 出各端是 worker 渲染前的前置。
...
> **蓝图对应:** 蓝图 `Per-Screen` 阶段每个 (屏×视口) worker 内部就是本段 3.0~3.5 的 **loop-until-converge**;`Synthesis` critic 干 3.6。
...
**停止判据 = 分数单调下降(变好)才继续修。** 对应蓝图 `Per-Screen` worker 内的 `for` 循环结构:
...
对应蓝图 `Synthesis` 阶段的单 critic agent:核覆盖后写两份 state(**只此一处写,保 key 严格对齐**):
...
## 主执行路径:AI 用内置 Workflow 工具按编排意图现场组合(推荐用户开 ultracode;降级=单 agent 顺序)

> ⚠️ **主路径到底怎么操作**:**Workflow 是 Claude Code 会话内的【工具】,由 AI(你)调用**(给它传 `script` 参数)——**不存在 `claude workflow` 这种 shell 命令**。**ultracode 是用户手动开的会话高级模式**(让 AI 默认倾向调 Workflow),skill/脚本无法自己开启它,只能推荐用户开。所以"执行本 skill 主路径"=你(AI)调用 **内置 Workflow 工具**,按本 skill 描述的编排意图现场组合 script 并执行;`scripts/design-first/design-restore.workflow.js` 只是【编排蓝图参考】,展示推荐扇出结构供你参考,不是被加载来运行的 script。脚本里的 agent 用 **Bash 调本仓 `scripts/design-first/` 的确定性脚本**(`visual-diff.mjs` / `token-match.mjs`)产出闸门读取的 state JSON——这些脚本是唯一可信的 state 产出口,不要让 agent 手写 JSON。

> **真 app 还原时,编排意图是扇出 + 每屏熔断,由 AI 用内置 Workflow 工具现场组合执行,不是单 agent 顺序硬写。** 单 agent 顺序跑只是降级档。

**为什么必须编排:** 一个 app 有 N 屏 × M 视口,顺序还原既慢又无并行覆盖;且每屏要 loop-until-converge(渲染→diff→定位→局部重生,分数单调降才继续),单 agent 没法既扇出又对每屏独立熔断。这套扇出编排把四质量模式落地:

- **fan-out 全覆盖** = `parallel(screen × viewport 笛卡尔积)`,每组合一个 worker,一屏不漏。
- **loop-until-converge** = screen worker 内 `for` + 分数单调降判据 + K=3 轮熔断。
- **completeness critic** = 蓝图 `Synthesis` 阶段单 agent 核覆盖(每屏每视口是否都跑了 pixelmatch+SSIM+token 对账、动态区是否都 mask、是否有屏未收敛漏报),**唯一写 state JSON 的点**,保证 key 严格对齐 `app-gate.sh`。

**怎么执行(在目标项目根):** AI 按上述编排意图,用内置 Workflow 工具现场组合扇出并执行。扇出结构可参考蓝图: `scripts/design-first/design-restore.workflow.js`。

**编排蓝图参考(已落盘 `scripts/design-first/design-restore.workflow.js`,此处内嵌全文供参考,展示推荐扇出结构;改了同步)**
...
**降级链(未做扇出编排 / .pen 不可达):**
1. 未做扇出编排(未开 ultracode 或不便扇出)→ 退线性段一~段三,单 agent 顺序跑,手写两份 state JSON(key 仍严格对齐)。
...
`完成: /design-restore 已按屏×视口扇出还原(AI 用内置 Workflow 工具执行),critic 写 ui-diff.json(mismatch=N)+ token-match.json(hardcoded=0/mismatched=0),设计已高保真还原`
...
`降级: 未做扇出编排(本会话非 ultracode),已退线性段一~段三单 agent 顺序跑,两份 state JSON 手写(key 仍对齐闸门)`
