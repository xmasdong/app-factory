...
### AI-易错产物 → 强类型中间表示 + 确定性编译器(高价值模式,来源:Three.js Object Sculptor 拆解 2026)

> **什么时候用**:要生成的产物是"AI 直出容易错、但可参数化"的东西——**程序化关卡/谜题、词库、地图、配置矩阵、
> 状态机、任何有硬约束(可解性/唯一性/引用完整)的结构化内容**。别让 AI 直接吐最终产物(易错且没法机械验)。

三段式(把 AI 从"做不好的低层生成"换成"填空 + 判断"):
1. **AI 只填强类型中间 spec**(可静态校验的 JSON/结构),不直出最终产物。
   例:关卡生成 → AI 填"网格尺寸/机制标签/目标态",不手摆每格;3D → 填"选哪个图元+参数",不吐顶点。
2. **确定性代码把 spec 编译成产物**(纯函数,可复现)。产物错 = spec 错或编译器错,可定位;不是"AI 手感飘了"。
3. **linter 校验 spec 自洽**(引用完整/枚举合法/深度达标/**硬约束成立**如"生成的关必可解")→ 不过不许进编译。

红利:① 改 spec 不用重生成一切(spec=SSOT,与产物解耦)② AI 干不了的部分**诚实退化 + TODO 标注**,不假装造好
③ 硬约束(可解性/去重/适龄)变成 linter 里的机械检查,不靠"AI 应该会注意"。
⚠️ 透镜非强制:简单静态内容(几条固定关卡)直写即可,别为 3 关搭编译器。产物**易错 × 量大 × 有硬约束**时才值。

### 证据锁步 = "完成"的定义(呼应 skill-ledger,来源同上)

分阶段产物(资产逐 pass 精化/关卡逐批生成/多屏逐个实现)里,一个阶段的"完成"**必须由可验证的证据工件喂出**——
截图 + 对比 + 分数达标 + 硬门通过,少一样就没完成,下一阶段物理上不该开工。**"完成"不是 AI 说完成,是留下了别人能复核的证据。** 这跟台账回执(⇒回执: path)、VLM 实证门是同一条铁律的不同切面。

### 自报分数硬封顶(反 Goodhart 再狠一层,VLM 实证门补强)

VLM/AI 给自己产物打分时,**对物理上达不到的精度设数值上限**,不许自报突破:
- 例:单张模糊参考图重建 → 不许声称 style_consistent=0.95(单图信息量封顶);证据不足的判断 → 置信度带天花板。
- 落地:验收记录里除了"过没过",加一句"本证据支持的最高可信分"——AI 自评超过它 = 无效,判未达标。
- 治的是"AI 为过门把自己分数往上够"这类自证。**证据打包(截图/对比图)与打分权威分离**:打包脚本只拼图不打分,判分才是判分。
