...
## Step 1.45: 能力反推 — 模块清单从产品推,不从模板铺

> 病根(oneshot/trade-copilot 实锤):自动 PRD 从"模板+5字段+gate清单"派生,倾向把"正经 app 该有的模块"
> (登录/paywall/广告/推送/订阅/记忆…)无脑铺满,而不是从**本产品核心能力/玩法**反推。
> 结果=模块在、闸门绿、但没绑真能力的摆设。模板驱动禁止盖过能力驱动。

拆任务前先答(写进 spec 决策清单):
1. **本产品的核心能力/玩法事件是什么?**(游戏=玩法循环节点;app=核心 user flow 节点)
2. **由此反推该有哪些模块** —— 通用件逐个过堂:"这个产品需要登录吗?需要 paywall 吗?需要推送吗?"
   不需要 → **显式删除**并记 Deferred(理由一句),不是留个壳
3. **留下的每个模块,ACCEPT 必须绑定真实能力/玩法事件**:
   - ❌ "广告服务文件存在 + 接口可调" → ✅ "插屏在第 N 关过关→下一关之间预载并展示,gameplay 中禁弹"
   - ❌ "分析服务返回 JSON" → ✅ "对 golden case 输入,输出达到参考质量"
   - 结构在+测试绿 ≠ 结合了本产品能力;验收句必须写"绑到哪个事件/哪条 flow"
...
## 游戏上架最小完整度(games 类 shape 必过一遍 —— OSR#1 实战教训)

> 首个毕业考交付被用户判"半成品":3 关、无设置页、触感粗糙。根因=shape 范围定太薄,build 忠实执行了窄范围。**范围薄不是 build 的锅,是 shape 没问这张单子**:

- **关卡量与曲线**:≥8 关渐进(首关 30 秒可懂;新压力源[限时/新机制]每 2-3 关引入一个,不在前 1/2 出现);曲线可单测(对数/par 单调性)。外部实证:Category Jam 新机制落在第 6/8/11/14/17/20 关,恰是 2-3 关节奏
- **进度系统**:解锁机制(过关开下一关)+ 每关成绩留存(星/分)—— 没有进度感 = 玩一次就删
- **设置页**(最低三件):触感开关 / 音效开关 / 重置进度;⚠️ 注明"模拟器无震动马达,触感真机才有感"
- **触感层级设计**(不是有没有,是节奏):高频动作用轻/脆(tick),达成用中,里程碑用重;**失败别用重震**(惩罚感)
- 以上写进任务清单,不是"以后再说"——它们是"能上架"和"半成品"的分界线

## 体验演出设计 EXPERIENCE-SCRIPT(games 类 shape 过一遍;app 的关键 flow 同理)

> **第三层缺位教训(drawguess R2)**:结构清单(上表)管"配齐没有",质感工序(build 7.3)管"件到位没有",
> 但"演出"——AI 思考要演戏、要有角色陪玩家、等待即叙事——是**设计物,checklist 长不出来**。
> 不前置设计,它就只能等用户玩到半成品才以反馈形式冒出来(= 白吃一轮 OSR)。
>
> ⚠️ **这些是联想方向(透镜),不是写死限制**:下面五条是"往这几个方向想过没有"的提问,
> 不是每游戏必配的配额。极简禅意解谜可以没有角色、快节奏街机可以没有思考戏——
> **想过并给一句理由的"不要",和压根没想过的"没有",是两回事**;不适用显式跳过,不凑数。
> 答案按本游戏现推,禁抄案例:

1. **节拍表**:把 core loop 拆成节点(开局/操作/等待/揭晓/胜利/失败/待机),每节点答三问:
   玩家做什么 → 游戏怎么**回应(演出)** → 玩家该感受到什么。答不出"感受"的节点 = 还没设计完
2. **等待即演出**:每个延迟/计算/加载节点禁裸转圈——问"这几秒游戏在'想'什么,能不能演出来?"
   (识别中=思考戏;生成关卡=铺场景;联网=角色小动作)
3. **情感载体**:这游戏有没有"谁"在陪玩家?(mascot/人格化语气/无——显式选择并给理由;
   儿童向默认要,且失败节拍=鼓励不是惩罚)
4. **内容×能力交叉校验**:内容清单(词库/关卡/谜题)必须与技术能力(端侧模型标签集/生成器可解范围)
   **取交集后再定稿**——内容超出能力 = 玩法死(出的题 AI 猜不中/生成的关解不开)。
   ⚠️ 程序化生成的内容(关卡/谜题/地图)= build 走「强类型中间 spec + 确定性编译器 + linter 验可解性」模式
   (见 build SKILL),别让 AI 直接吐关卡;shape 阶段就把"可解性/去重/适龄"定义成 linter 能查的硬约束。
5. **自检金句**:随机录 10 秒屏给陌生人看,有没有一个"活的"瞬间?没有 = 演出层还是空的
6. **关内节拍曲线(单局张弛)**:一关之内也有多巴胺曲线——爆款样本(Category Jam,2026)把单关拆成
   "先易(清障上手)→ 聚焦(有限空间里找/判断,难度峰)→ **释放段(通关前一段无脑快速收割,情绪峰值)**",
   一张一弛,峰终律让通关记忆极强。问本游戏:单关结尾有没有设计"释放段"?难度峰在中段还是错put在结尾?
   反面镜(同文):表现大于策略 = 忌(3D 模拟感再好,策略只剩"寻找"就输了);融合机制若拖慢节奏 = 减分不是加分。
   表现服务玩法,永远。
- 产物:spec 加 `## EXPERIENCE-SCRIPT` 节(节拍表);演出项直接变任务 + 喂 DESIGN-FEED 资产清单
  (mascot 表情数/演出用件);mockup 里体现 1-2 个关键演出帧,让用户 TOUCH-2 就能对演出拍板
