...
### 需求证据强度阶梯(选品排序权重;外部实证:Ones 案例,星夜AI出海 2026)

调研证据不是等价的,越靠上越值钱——**能拿到上层证据时,上层 > 下层的猜**:

1. **真实承诺(最硬)**:真的人在真实场景做出承诺——付费/占日期/把产品塞进真实活动。
   Ones 的标尺=**先敲定 10 场有明确日期的活动再写代码**(不是"找 10 个人聊聊",是"对方愿意把一段真实生活交给你")。
   能做承诺验证的品类(活动/工具/垂直 B2B),这一条抵过一切榜单数据。
2. **在用竞品的真金**:付费榜位/真实营收指纹/差评里的"我愿意为 X 付钱"
3. **需求信号**:搜索量/差评量/社区抱怨密度(本 Step 上面 6 项主要产这层)
4. **推断(最软)**:"这个方向应该有人要"——训练记忆式自证,最不算数

- **具体场景 > 功能列表**:产品要有"具体到能被人记住的场景"(Ones 抓的不是"拍照",是"大家一起等照片被揭晓的瞬间")。
  5 字段的 TARGET_USER 若写不出一个具体到能被记住的使用场景 = 还没想清楚,别急着往下。
- **不要一上来做平台**:先找窄到能验证的点。答不上"谁会用/为什么现在用/不用有什么损失/愿不愿付费或付时间"
  = 代码写得越快只是越快做个没人要的东西。
- ⚠️ 透镜非配额:承诺验证不是每个品类都做得到(冷启动 ASO 游戏难约 10 个真实用户);做不到就诚实降到第 2/3 层,
  但**别把"做不到承诺验证"偷换成"不需要验证"**。
