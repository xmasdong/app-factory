...
$AI_RULES_ROOT 用法: 测试命令脚本 / 断言清单 / 越权矩阵模板落在 $AI_RULES_ROOT 下, 项目间可移植.
  - 契约测试: $AI_RULES_ROOT/scripts/design-first/contract-test.sh   (封装 schemathesis, 透传 --target)
  - E2E smoke: $AI_RULES_ROOT/scripts/design-first/e2e-contract.sh
...
$AI_RULES_ROOT/scripts/design-first/contract-test.sh --target mock \
...
$AI_RULES_ROOT/scripts/design-first/contract-test.sh --target real \
  --schema api/openapi.yaml \
...
> 断言落在 $AI_RULES_ROOT/scripts/design-first/e2e-contract.sh, 对 real 后端跑一遍核心用户流后逐字段校验.
...
$AI_RULES_ROOT/scripts/design-first/contract-test.sh --target real --negative-ownership \
  --schema api/openapi.yaml \
...
| 字段对照 (E2E smoke) | e2e-contract.sh | 关键字段语义约束 (分/ISO/枚举) | ≠ 流程对 |
