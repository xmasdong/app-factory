...
' "$STATUS_FILE"
  ...
  jq -nc '{
    systemMessage: "[app-factory autonomous] 用户已确认推进. status.md 已写 AUTONOMOUS=true + CURRENT_GATE=A-GATE Lockdown. 立即进入 /lockdown skill, 不再询问决策. 全自动跑到 ship."
  }' 2>/dev/null
  exit 0
fi
...
if echo "$PROMPT" | grep -qiE '^[[:space:]]*(换方向|换个|换思路|不行|换|reject|另选|重新探索|换 ?[a-z]+)'; then
  ...
  jq -nc --arg msg "[app-factory] $MSG_CORE" '{ systemMessage: $msg }' 2>/dev/null
  exit 0
fi
...
if echo "$PROMPT" | grep -qiE '^[[:space:]]*(暂停|pause|稍等|wait|hold|放一下)'; then
  jq -nc '{
    systemMessage: "[app-factory] 用户要求暂停. 不动任何状态. 等用户回来再说."
  }' 2>/dev/null
  exit 0
fi
