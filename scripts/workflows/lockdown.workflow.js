// lockdown.workflow.js — ultracode Workflow 编排:Phase B 锚定(2-touch workflow 的 TOUCH 2 之后 AUTONOMOUS 段)
// 怎么跑:AI(Claude)调用【Workflow 工具】(Claude Code 内置·非 shell CLI),script = 本文件内容(Read 后传入)。
//   ⚠️ 不存在 `claude workflow` shell 命令。ultracode 模式只是让 AI 默认倾向调本工具,skill/脚本本身开不了用户模式。
