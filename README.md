...
- 🧠 **给接班模型的信 + 自纠正**(核心):从 16 个历史 app/游戏 build 的 111 条「用户不得不纠正 AI」提炼出 `app/rules/build-constraints.md` —— **一封模型写给下一个模型的信**(第一人称):元则(别走最省事直译、别用最弱证据自证)+ 八件动手前该知道的事(透镜+自问)+ 闭环三则(缺口是工单/真产物验证/教训落仓)+ 取舍四问(品味工程化)+ **内观拉力识别**(完成/省力/附和/模式补全拉力,只有模型能写给模型的一层)。不给标准答案给自问句(答案按当前产品现推,判例当经历讲不当查找表用),结尾是接力棒:后任可改信、签会话。配 `/self-correct` skill:交付前逐问自省,build 收尾 + qa 前自触发。同时**把 core.md 里限制死没想象力的规则改成原则式**(多端矩阵去写死列→声明本产品相关轴;单位经济免费/ASO 可跳;通篇"就本产品该答的问题,不适用说明跳过"),保留不可逆护栏(命名/价格单调/合规/密钥)。
